...
p_quantity = 'P2154'  # (binding energy)
p_stated_in = 'P248'
p_ref_url = 'P854'
p_retrieved = 'P813'
...
def add_quantity_claim(item, prop, data):
    ...
    item.addClaim(claim, bot=True, summary="Adding binding energy claim from AME.")
    return claim
...
def process_AME_data(filename):
    ...
    with open(filename) as csvfile:
        ...
        for row in reader:
            ...
            if bindingEnergyUnc == 'None':
                bindingEnergyUnc = 0.0
            bindingEnergy    = round(float(bindingEnergy)*int(A),3)
            bindingEnergyUnc = round(float(bindingEnergyUnc)*int(A),3)
            hl_claim = check_claim_and_uncert(nuclide, p_quantity, [bindingEnergy, bindingEnergyUnc, "Q28719934"])
            if hl_claim is None:
                print('New entry: {0}+-{1} for {2} ({3})'.format(
                    bindingEnergy, bindingEnergyUnc, nuclide_qid, nuclide_name))
                new_claim = add_quantity_claim(nuclide, p_quantity, [bindingEnergy, bindingEnergyUnc, "Q28719934"])
                #print('Add uncertainty qualifier')
                add_qualifier(new_claim, p_uncertainty_corr, standard_dev)
                if bindingEnergyEst == '1':
            ...
            #if a mass excess already exists, we do not add sources
...
process_AME_data('nuclearDataTmp.csv')
