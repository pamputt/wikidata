...
def process_AME_data(filename):
    ...
    with open(filename) as csvfile:
        ...
        for row in reader:
            ...
            if massExcessUnc == 'None':
                massExcessUnc = 0.0
            hl_claim = check_claim_and_uncert(nuclide, p_quantity, [massExcess, massExcessUnc, "Q28719934"])
            if hl_claim is None:
                print('New entry: {0}+-{1} for {2} ({3})'.format(
                    massExcess, massExcessUnc, nuclide_qid, nuclide_name))
                new_claim = add_quantity_claim(nuclide, p_quantity, [massExcess, massExcessUnc, "Q28719934"])
